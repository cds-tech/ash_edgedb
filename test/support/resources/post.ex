defmodule AshEdgeDB.Test.Post do
  @moduledoc false
  use Ash.Resource,
  ...
  attributes do
    ...
    attribute(:status, AshEdgeDB.Test.Types.Status)
    attribute(:status_enum, AshEdgeDB.Test.Types.StatusEnum)
    attribute(:status_enum_no_cast, AshEdgeDB.Test.Types.StatusEnumNoCast, source: :status_enum)
    attribute(:point, AshEdgeDB.Test.Point)
    attribute(:stuff, :map)
  ...
  relationships do
    ...
    has_many(:comments, AshEdgeDB.Test.Comment, destination_attribute: :post_id)

    has_many :comments_matching_post_title, AshEdgeDB.Test.Comment do
      filter(expr(title == parent_expr(title)))
    end

    has_many :popular_comments, AshEdgeDB.Test.Comment do
      destination_attribute(:post_id)
      filter(expr(likes > 10))
    end

    has_many :comments_containing_title, AshEdgeDB.Test.Comment do
      manual(AshEdgeDB.Test.Post.CommentsContainingTitle)
    end

    has_many :comments_with_high_rating, AshEdgeDB.Test.Comment do
  ...
  calculations do
    ...
    calculate :similarity,
      ...
      argument(:search, AshEdgeDB.Tsquery, allow_expr?: true, allow_nil?: false)
    end

    calculate :query, AshEdgeDB.Tsquery, expr(fragment("to_tsquery(?)", ^arg(:search))) do
      argument(:search, :string, allow_expr?: true, allow_nil?: false)
    end

    calculate(
      :calc_returning_json,
      AshEdgeDB.Test.Money,
      expr(
        fragment("""
  ...
  aggregates do
    ...
    custom(:comment_authors, [:comments, :author], :string) do
      implementation({AshEdgeDB.Test.StringAgg, field: :first_name, delimiter: ","})
    end
