defmodule AshEdgeDB.MigrationGenerator do
  @moduledoc false
  ...
  alias AshEdgeDB.MigrationGenerator.{Operation, Phase}
  ...
  def generate(apis, opts \\ []) do
    ...
    {tenant_snapshots, snapshots} =
      ...
      |> Enum.filter(fn resource ->
        Ash.DataLayer.data_layer(resource) == AshEdgeDB.DataLayer &&
          AshEdgeDB.DataLayer.Info.migrate?(resource)
      end)
  ...
  def take_snapshots(api, repo, only_resources \\ nil) do
    ...
    |> Enum.filter(fn resource ->
      Ash.DataLayer.data_layer(resource) == AshEdgeDB.DataLayer &&
        AshEdgeDB.DataLayer.Info.repo(resource, :mutate) == repo &&
  ...
  defp add_custom_index_name(custom_index, table) do
    ...
    |> Map.put_new_lazy(:name, fn ->
      AshEdgeDB.CustomIndex.name(table, %{fields: custom_index.fields})
    end)
    |> Map.update!(
      :name,
      &(&1 || AshEdgeDB.CustomIndex.name(table, %{fields: custom_index.fields}))
    )
  end
  ...
  def get_snapshots(resource, all_resources) do
    Code.ensure_compiled!(AshEdgeDB.DataLayer.Info.repo(resource, :mutate))
    ...
    if AshEdgeDB.DataLayer.Info.polymorphic?(resource) do
      ...
      |> Enum.map(fn relationship ->
        ...
        |> Map.update!(:identities, fn identities ->
          identity_index_names = AshEdgeDB.DataLayer.Info.identity_index_names(resource)
              ...
              Map.put(attribute, :references, %{
                ...
                destination_attribute_default:
                  default(
                    source_attribute,
                    relationship.destination,
                    AshEdgeDB.DataLayer.Info.repo(relationship.destination, :mutate)
                  ),
                deferrable: false,
                destination_attribute_generated: source_attribute.generated?,
                multitenancy: multitenancy(relationship.source),
                table: AshEdgeDB.DataLayer.Info.table(relationship.source),
                schema: AshEdgeDB.DataLayer.Info.schema(relationship.source),
                on_delete: AshEdgeDB.DataLayer.Info.polymorphic_on_delete(relationship.source),
                on_update: AshEdgeDB.DataLayer.Info.polymorphic_on_update(relationship.source),
                primary_key?: source_attribute.primary_key?,
                name:
                  AshEdgeDB.DataLayer.Info.polymorphic_name(relationship.source) ||
      ...
      [do_snapshot(resource, AshEdgeDB.DataLayer.Info.table(resource))]
    end
  end
  ...
  defp do_snapshot(resource, table, schema \\ nil) do
    snapshot = %{
      ...
      table: table || AshEdgeDB.DataLayer.Info.table(resource),
      schema: schema || AshEdgeDB.DataLayer.Info.schema(resource),
      ...
      repo: AshEdgeDB.DataLayer.Info.repo(resource, :mutate),
      multitenancy: multitenancy(resource),
      base_filter: AshEdgeDB.DataLayer.Info.base_filter_sql(resource),
  ...
  defp check_constraints(resource) do
    ...
    |> case do
      ...
      constraints ->
        ...
        if base_filter && !AshEdgeDB.DataLayer.Info.base_filter_sql(resource) do
          raise """
  ...
  defp custom_indexes(resource) do
    ...
    |> Enum.map(fn custom_index ->
      Map.take(custom_index, AshEdgeDB.CustomIndex.fields())
    end)
  end

  defp custom_statements(resource) do
    resource
    |> AshEdgeDB.DataLayer.Info.custom_statements()
    |> Enum.map(fn custom_statement ->
      Map.take(custom_statement, AshEdgeDB.Statement.fields())
    end)
  end
  ...
  defp attributes(resource, table) do
    repo = AshEdgeDB.DataLayer.Info.repo(resource, :mutate)
    ignored = AshEdgeDB.DataLayer.Info.migration_ignore_attributes(resource) || []
    ...
    |> Enum.map(fn attribute ->
      ...
      type =
        AshEdgeDB.DataLayer.Info.migration_types(resource)[attribute.name] ||
      ...
      if attribute.source == source_attribute_name && relationship.type == :belongs_to &&
        ...
        unless Map.get(configured_reference, :ignore?) do
          ...
          %{
            ...
            schema:
              relationship.context[:data_layer][:schema] ||
                AshEdgeDB.DataLayer.Info.schema(relationship.destination) ||
                AshEdgeDB.DataLayer.Info.repo(relationship.destination, :mutate).config()[
                  :default_prefix
                ],
            table:
              relationship.context[:data_layer][:table] ||
                AshEdgeDB.DataLayer.Info.table(relationship.destination)
          }
        end
      end
    end)
  ...
  defp foreign_key?(relationship) do
    Ash.DataLayer.data_layer(relationship.source) == AshEdgeDB.DataLayer &&
      AshEdgeDB.DataLayer.Info.repo(relationship.source, :mutate) ==
        AshEdgeDB.DataLayer.Info.repo(relationship.destination, :mutate)
  end
  ...
  defp identities(resource) do
    ...
    |> Enum.reject(fn identity ->
      identity.name in AshEdgeDB.DataLayer.Info.skip_unique_indexes(resource)
    end)
    ...
    |> Enum.map(fn identity ->
      Map.put(
        ...
        :index_name,
        identity_index_names[identity.name] ||
          "#{AshEdgeDB.DataLayer.Info.table(resource)}_#{identity.name}_index"
      )
    end)
    |> Enum.map(&Map.put(&1, :base_filter, AshEdgeDB.DataLayer.Info.base_filter_sql(resource)))
  end
  ...
  defp configured_default(resource, attribute) do
    AshEdgeDB.DataLayer.Info.migration_defaults(resource)[attribute]
  end
